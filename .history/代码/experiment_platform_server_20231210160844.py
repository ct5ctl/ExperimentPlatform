...
async def send_message(websocket):
    ...
    while True:
        ...
        pos_current = [116.38553266, 39.90440998, 0]
        theta_current = 270
...
def websocket_server(q, name):
    while True:
        # 从队列中获取数据
        pos_current = q.get()
        print(f"websocket_server pos_current:", pos_current)
        # 通过websocket发送数据
...
if __name__ == "__main__":
    ...
    # 启动写入数据的进程
    writer_process = multiprocessing.Process(target=pos_server, args=(q,))
    writer_process.start()

    # 启动两个读取数据的进程
    reader1_process = multiprocessing.Process(target=websocket_server, args=(q, "1"))
