...
def geodetic_to_ecef(lon, lat, h):
    ...
    N = a_E / w
    X = N + h * Decimal(math.cos(lat)) * Decimal(math.cos(lon))
    Y = (N + h) * Decimal(math.cos(lat)) * Decimal(math.sin(lon))
    Z = (N * (1 - e2) + h) * Decimal(math.sin(lat))
...
def send_simul_start_command(q_pos, q_theta, simula_data):
    ...
    x, y, z = geodetic_to_ecef(float(pos_current[0]), float(pos_current[1]), float(pos_current[2]))
