...
# ===================================线程3子任务===================================
def simul_start_command(q):
    print("已发送导航模拟启动指令")
    pass

def track_data_command(q):
    print("已发送轨迹数据指令")
    pass
...
def navigation_simulation_server(q, flag):
    while True:
        if flag.is_set():
            # 非首次执行，发送轨迹数据指令
            send——track_data_command(q)
        else:
            # 首次执行，发送导航模拟启动指令
            simul_start_command(q)
            flag.set()  # 设置标志，以便执行任务 b
        
        time.sleep(0.1)  # 轨迹发送频率
def navigation_simulation_server(q, name):
    while True:
        # 从队列中获取数据
        data = q.get()
...
if __name__ == "__main__":
    ...
    websocket_server_process = multiprocessing.Process(target=start_websocket_server, args=(q, flag))
