...
def calculate_next_pos_theta(last_moment_pos, last_moment_theta, speed, wheel_angle, wheel_base=2.785, time_slot=0.1):
    ...
    # 计算经纬度增量
    delta_longitude = math.degrees(dsp_x / (earth_radius_km * math.cos(math.radians(last_moment_pos[1]))))
    delta_latitude = math.degrees(dsp_y / earth_radius_km)

    # 计算下一时刻的位置
    next_pos = [last_moment_pos[0] + delta_longitude, last_moment_pos[1] + delta_latitude, 0]
