...
def pos_server(q):
    while True:
        ...
        pos_current, theta_current = process_sensor_data(sensor_data, vehicle_data)  # 处理传感器数据
        
        # 写入数据到队列
        q.put(pos_current)
        print(str(pos_current))
        print(f"writer data:", a)
...
if __name__ == "__main__":
    ...
    # 启动两个读取数据的进程
    reader1_process = multiprocessing.Process(target=reader1, args=(q, "1"))
