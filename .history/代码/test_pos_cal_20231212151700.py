...
def calculate_next_pos_theta(last_moment_pos, last_moment_theta, speed, wheel_angle, wheel_base, time_slot):
    # 将角度转换为弧度
    last_moment_theta_rad = math.radians(last_moment_theta)
    
    # 车辆行驶的距离
    distance = (speed / 3.6) * time_slot  # 将速度从 km/h 转换为 m/s
    
    # 计算车辆下一时刻的位置
    next_pos = (
        last_moment_pos[0] + distance * math.cos(last_moment_theta_rad),
        last_moment_pos[1] + distance * math.sin(last_moment_theta_rad)
    )
    
    # 计算车辆下一时刻的朝向角度
    turning_radius = wheel_base / math.sin(math.radians(wheel_angle))
    delta_theta = (distance / turning_radius) * math.cos(math.radians(wheel_angle))
    next_theta = (last_moment_theta + math.degrees(delta_theta)) % 360
