...
# 测试函数
def test_calculate_next_pos_theta():
    # 初始位置
    initial_pos = [0, 0, 0]  # 经度、纬度、高度
    initial_theta = 90  # 初始航向角度
    
    # 车辆参数
    wheel_base = 2.875  # 车辆轴距
    
    # 模拟输入的轮角度和速度
    wheel_angle = 10  # 假设车轮角度为10度
    speed = 50  # 假设速度为50 km/h
    
    # 模拟时间间隔
    time_slot = 0.1  # 时间间隔为0.1秒
    
    # 计算下一个位置和航向角
    next_pos, next_theta = calculate_next_pos_theta(initial_pos, initial_theta, speed, wheel_angle, wheel_base, time_slot)
    
    print("初始位置：", initial_pos)
    print("初始航向角度：", initial_theta)
    print("模拟时间间隔：", time_slot)
    print("模拟车速：", speed, "km/h")
    print("模拟车轮角度：", wheel_angle, "度")
    print("下一位置：", next_pos)
    print("下一航向角度：", next_theta)

# 运行测试
test_calculate_next_pos_theta()