...
def send_simul_start_command(q_pos, simula_data):
...
def send_track_data_command(q_pos):
    pos_current = q_pos.get()
    print("已发送轨迹数据指令")
    pass
...
def pos_server(q_pos, vehicle_data, log_file):
    while True:
...
def navigation_simulation_server(q_pos, flag, simula_data):
    while True:
        if flag.is_set():
            # 非首次执行，发送轨迹数据指令
            send_track_data_command(q_pos)
        else:
            # 首次执行，发送导航模拟启动指令
            send_simul_start_command(q_pos, simula_data)
            flag.set() 
...
if __name__ == "__main__":
    ...
    # 启动车辆数据预测进程
    pos_server_process = multiprocessing.Process(target=pos_server, args=(q_pos, vehicle_data, log_file))
    ...
    flag.clear()  
    # 启动导航模拟
    navigation_simulation_process = multiprocessing.Process(target=navigation_simulation_server, args=(q_pos, flag, simula_data))
